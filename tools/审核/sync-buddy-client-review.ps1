...
try {
    ...
    robocopy `
        ...
        /E `
        /XO `
        /XD ".git" "node_modules" "dist" "build" ".next" ".turbo" ".vite" ".nuxt" "coverage" ".expo" `
        /R:2 `
        /W:2 `
        /FFT `
        /Z `
    ...
    Write-Host ""
    Write-Host "Robocopy Exit Code: $RoboCode"

    if ($RoboCode -le 7) {
        Write-Host "Sync completed successfully." -ForegroundColor Green
        exit 0
    } else {
        Write-Host "Sync failed. Check log file." -ForegroundColor Red
        exit $RoboCode
    }

}
catch {
    Write-Host "Sync exception occurred." -ForegroundColor Red
